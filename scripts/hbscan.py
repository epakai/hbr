...
def parse_args (args=sys.argv[1:]):
    ...
    parser.add_argument('--minlength', metavar='SECONDS', default=0,
            type=int, help="Minimum length for a title to be include in output")
    parser.add_argument("--setiso", action='store_true',
            dest='setiso', help='include iso_filename in outfile sections')
    parser.add_argument("-", action='store_true',
            dest='read_stdin', help='read from stdin')
...
"""
lines
   general output (no good identifier)
   log output     (starts with time code in brackets)
   progess update (starts with )
   title info     (+ following space indentation)
        title header
        playlist
        duration
        size
...
class Audio(object):
    def __init__(self, audio_number, codec, channels, iso639, frequency, bitrate):
...
def add_subtitle(title_list):
    def parseAction(string, loc, t):
        title_list[-1].subtitle.append(
                Subtitle(t.sub_number, t.sub_lang, t.sub_format))
    return parseAction
...
def buildParser(title_list):
   ...
   '''
    ...
    time = (pp.Combine(pp.Word(pp.nums, exact=2) 
        ...
        + ':' + pp.Word(pp.nums, exact=2)))
    log_output = '[' + time("time") + ']' + pp.OneOrMore(pp.Word(pp.printables))
    ...
    sizes = ('+ size:' + pp.Combine(pp.Word(pp.nums) + 'x' + pp.Word(pp.nums))( "resolution") 
    ...
    audio = ('+' 
            ...
            + pp.OneOrMore(pp.Word(unicodePrintables, excludeChars='()'))("audio_lang")
            + pp.Combine('(' + pp.Word(pp.alphanums + '- ')("audio_codec") + ')') 
            ...
            + pp.Optional(pp.Combine(pp.Word(pp.nums) + 'bps')("audio_bitrate")))
    audio.setParseAction(add_audio(title_list))

    subtitle = ('+' 
            + pp.Word(pp.nums)("sub_number") 
            + pp.Combine(pp.OneOrMore(pp.Word(unicodePrintables, excludeChars=',([')))("sub_lang")
    ...
    line = title_path | log_output | progress_update | main_feature | combing | subtitle | audio | chapter | autocrop | playlist | duration | sizes | title_number | title_headers | title_info | general_output | empty
...
def emit_outfile_sections(title_list):
    ...
    for title in title_list:
        ...
        seconds = sum([a*b for a,b in zip(ftr, map(int,title.duration.split(':')))])
        ...
        if args.setiso:
            print("iso_filename=", os.path.basename(os.path.normpath(title.filename)), sep='')
        print('specific_name=')
        if title.playlist == None:
            print('#', title.duration)
        elif title.filename == None:
            print('#', title.playlist.lower(), title.duration)
        else:
            mpls_file_path = os.path.join(title.filename, "BDMV", "PLAYLIST", title.playlist.lower())
            mpls_dump_out = subprocess.run(['mpls_dump', '-l', mpls_file_path],
                    stdout=subprocess.PIPE, cwd=os.getcwd(), text=True,
            ...
            print('#', title.playlist.lower(), title.duration, '(', ', '.join(m2ts_list), ')')
        ...
        if title.audio:
            ...
            print('audio=', ','.join([audio.audio_number for audio in title.audio]), sep='')
        if title.subtitle:
            print('# ', ','.join([str(subtitle.language + ' [' +
                subtitle.sub_format + ']') for subtitle in title.subtitle]), sep='')
            print('subtitle=', ','.join([subtitle.subtitle_number for subtitle in title.subtitle]), sep='')
...
def main():
    ...
    if (args.read_stdin):
        ...
        for line in sys.stdin.readlines():
            ...
            if (len(title_list) > 0 and title_list[-1].filename == None and
                    current_path != None):
        ...
        for path in args.path:
            ...
            hb_out = subprocess.run(['HandBrakeCLI', '--scan', '-t', '0', '-i', path],
                    stdout=subprocess.PIPE, stderr=subprocess.STDOUT, cwd=os.getcwd())
            # Strip libdvdnav lines because they have iso-8859-1 encodings that are
            # not valid utf-8, we don't need them for our purpose.
            output = b'\n'.join(line for line in hb_out.stdout.splitlines()
                    if not line.startswith(b'libdvdnav:')).decode('utf-8')
            for line in output.splitlines():
                if (len(title_list) > 0 and title_list[-1].filename == None):
