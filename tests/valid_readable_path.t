Good readable path (input_basedir=/)
  $ "$CRAM_HBR" "$CRAM_HBR_ARGS" -d -c "$TESTDIR"/configs/empty "$TESTDIR"/valid_readable_path/good.hbr 2>&1 | sed 's@'"$TESTDIR"'@TESTDIR@g'
...
Bad readable path (input_basedir=)
  $ "$CRAM_HBR" "$CRAM_HBR_ARGS" -d -c "$TESTDIR"/configs/empty "$TESTDIR"/valid_readable_path/bad.hbr 2>&1 | sed 's@'"$TESTDIR"'@TESTDIR@g'
