Writable directory in hbr file (/tmp)
  $ "$CRAM_HBR" -d -c "$TESTDIR"/configs/empty "$TESTDIR"/valid_writeable_path/good.hbr 2>&1 | sed 's@'"$TESTDIR"'@TESTDIR@g'
...
Non-writable directory in hbr file (empty key)
  $ "$CRAM_HBR" -d -c "$TESTDIR"/configs/empty "$TESTDIR"/valid_writeable_path/bad.hbr 2>&1 | sed 's@'"$TESTDIR"'@TESTDIR@g'
...
Writable directory passed on command line (-o /tmp)
  $ "$CRAM_HBR" -d -o /tmp -c "$TESTDIR"/configs/empty "$TESTDIR"/valid_writeable_path/good.hbr 2>&1 | sed 's@'"$TESTDIR"'@TESTDIR@g'
  \x1b[1m# Encoding: 1/1: good (2000).mkv (esc)
  \x1b[0mHandBrakeCLI --title=1 -i '/test.iso' -o '/tmp/good (2000).mkv' (esc)

Non-writable directory passed on command line (-o /)
  $ "$CRAM_HBR" -d -o / -c "$TESTDIR"/configs/empty "$TESTDIR"/valid_writeable_path/good.hbr 2>&1 | sed 's@'"$TESTDIR"'@TESTDIR@g'
