...
Movie outfile with no year (should warn)
  $ "$CRAM_HBR" -d -c "$TESTDIR"/configs/empty "$TESTDIR"/valid_type/movie_no_year.hbr 2>&1 |sed 's@'"$TESTDIR"'@TESTDIR@g'
...
Series outfile with no episode, season, and neither (should warn in all three cases)
  $ "$CRAM_HBR" -d -c "$TESTDIR"/configs/empty "$TESTDIR"/valid_type/bad_series.hbr 2>&1 |sed 's@'"$TESTDIR"'@TESTDIR@g'
...
movie with no year (warning), then a good example
  $ "$CRAM_HBR" -d -c "$TESTDIR"/configs/empty "$TESTDIR"/valid_type/type_varies.hbr 2>&1 |sed 's@'"$TESTDIR"'@TESTDIR@g'
