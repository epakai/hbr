Good Series Input
  $ "$CRAM_HBR" "$CRAM_HBR_ARGS" -d -c "$TESTDIR"/configs/empty "$TESTDIR"/valid_type/good_series.hbr
  \x1b[1m# Encoding: 1/1: A - s01e001.mkv (esc)
  \x1b[0mHandBrakeCLI --title=1 -i '/test.iso' -o 'A - s01e001.mkv' (esc)

Good Movie Input
  $ "$CRAM_HBR" "$CRAM_HBR_ARGS" -d -c "$TESTDIR"/configs/empty "$TESTDIR"/valid_type/good_movie.hbr
...
Movie outfile with no year (should warn)
  $ "$CRAM_HBR" "$CRAM_HBR_ARGS" -d -c "$TESTDIR"/configs/empty "$TESTDIR"/valid_type/movie_no_year.hbr 2>&1 |sed 's@'"$TESTDIR"'@TESTDIR@g'
...
Series outfile with no episode, season, and neither (should warn in all three cases)
  $ "$CRAM_HBR" "$CRAM_HBR_ARGS" -d -c "$TESTDIR"/configs/empty "$TESTDIR"/valid_type/bad_series.hbr 2>&1 |sed 's@'"$TESTDIR"'@TESTDIR@g'
...
movie with no year (warning), then a good example
  $ "$CRAM_HBR" "$CRAM_HBR_ARGS" -d -c "$TESTDIR"/configs/empty "$TESTDIR"/valid_type/type_varies.hbr 2>&1 |sed 's@'"$TESTDIR"'@TESTDIR@g'
